...
unsafe fn modify_by_address(address: usize) {
    // SAFETY:
    // The caller has guaranteed that `address` is valid, non-null,
    // aligned and refers to a unique mutable u32. We cast the address
    // to a mutable pointer and write a new value to it.
